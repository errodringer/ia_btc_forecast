...
X, y = np.array(scaled_prices[:-1]), np.array(signals)
...
# ])
model = Sequential([
    LSTM(50, input_shape=(X_train.shape[0], 1)),
    Dense(3, activation='softmax')
])
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

# Entrenamos el modelo
model.fit(X_train, y_train_cat, epochs=10, batch_size=X_train.shape[0], validation_data=(X_test, y_test_cat))
...
plt.plot(prices.index[-len(y_test):], scaler.inverse_transform(X_test[:, -1].reshape(-1, 1)), label='Precio de Cierre')
